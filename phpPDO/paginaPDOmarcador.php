...
<body>
  <h3>Pagina de consulta con PHP - PDO </br>EJEMPLO CON MARCADORES</h3>
    <?php
    $buscar=$_GET["buscar"];
    try {
        ...
        , visita_tecnica, descripcion_evento, tecnico_ctk, codigo_cliente FROM ctk_ticket where tecnico_ctk=:in_tec";
        $resultado= $base->prepare($sql);//varible resultado almacena el objeto de tipo PDO statement
        //el objeto base(conexión)
        $resultado->execute(array("in_tec"=>$buscar));
